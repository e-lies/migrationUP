...
const Charts = (props) => {
  ...
  useEffect(()=>{
    console.log("propTypes = ", schema, data)
    if(chartParams){
  ...
  const pieUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.label && obj.value) {  
      var dt = newSchema.columns[obj.label]['type'].includes('foreign')
        ? data.map(d=>{
          return {...d,
            [obj.label]:newSchema.columns[obj.label].possibles.find(p=>{return p.value===d[obj.label]}) ? newSchema.columns[obj.label].possibles.find(p=>{return p.value===d[obj.label]})['label'] : d[obj.label]
            }
          })
        : data
  ...
  const radarUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.group && obj.index && obj.Y && obj.op) {
      var dt = newSchema.columns[obj.group]['type'].includes('foreign')
        ? data.map(d=>{
          return {...d,
            [obj.group]:newSchema.columns[obj.group].possibles.find(p=>{return p.value===d[obj.group]}) ? newSchema.columns[obj.group].possibles.find(p=>{return p.value===d[obj.group]})['label'] : d[obj.group]
            }
          })
        : data
  ...
  const matrixUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.group && obj.index && obj.Y && obj.op) {
      var dt = newSchema.columns[obj.index]['type'].includes('foreign')
        ? data.map(d=>{
          return {...d,
            [obj.index]:newSchema.columns[obj.index].possibles.find(p=>{return p.value===d[obj.index]}) ? newSchema.columns[obj.index].possibles.find(p=>{return p.value===d[obj.index]})['label'] : d[obj.index]
            }
          })
        : data
      const formattedData = clientBMR(dt, obj.group, obj.index, obj.Y, obj.op);
  ...
  const barUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.X !== null && obj.Y !== null && obj.op !== null) {
      ...
      if(newSchema.columns[obj.X]['type'].includes('foreign')){
        dt = dt.map(d=>{
          return {...d,
            [obj.X]:newSchema.columns[obj.X].possibles.find(p=>{return p.value===d[obj.X]}) ? newSchema.columns[obj.X].possibles.find(p=>{return p.value===d[obj.X]})['label'] : d[obj.X]
            }
          })
      }
  ...
  const lineUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.X && obj.Y && obj.op) {
      ...
      ? data.map(d=>{
        return {...d,
          [obj.id]:newSchema.columns[obj.id].possibles.find(p=>{return p.value===d[obj.id]}) ? newSchema.columns[obj.id].possibles.find(p=>{return p.value===d[obj.id]})['label'] : d[obj.id]
          }
        })
      : data
