        ...
        <Lista 
            ...
            subheader={(
                <div style={{display:'flex'}}>
                    {checkedFunctions && (
                        <Checkbox
                            color={checked.length > 0 ? 'error' : 'default'}
            ...
            {data.map((list,i)=>{
                ...
                <div key={`listItem${i}`} disabled={!draggable} /*dragHandle={<DragHandle/>}*/>
                    <ListItem id={`listItem${list[idKey]}`} style={itemStyle} onClick={()=>onClick(list)}>
                        {checkedFunctions && (
                            <Checkbox
                                color={checked.includes(list[idKey]) ? 'error' : 'default'}
                        ...
                        <ListItemText 
                            id={`text${list[idKey]}`}
                            primary={primary ? typeof primary === 'string' ? `${list[primary]}${schema.columns[primary].suffixe || ''}` : primary(list) : null} 
                            secondary={secondary ? typeof secondary === 'string' ? `${list[secondary]}${schema.columns[secondary].suffixe || ''}` : secondary(list) : null}
                        />
                        { action && (
                            <div style={{maxWidth:'50%',display:'flex',flexWrap:'wrap'}}>
                                { action(list) }
                            </div>
                        )}
