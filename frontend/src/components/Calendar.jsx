...
import Theme from "../Themes/Microsoft";
...
function CalendarUnit({
  ...
  React.useEffect(() => {
    ...
    const parent = document.getElementById("calendar-sidebar").firstChild
      .firstChild;
  ...
  useEffect(() => {
    document.querySelector("#month").innerText =
      new Date(date).toDateString().slice(4, 7) +
      new Date(date).toDateString().slice(10, 15);
  }, [date]);
  React.useEffect(() => {
    setDv(data);
  }, [JSON.stringify(data)]);
    ...
    return (
      <div className="rbc-toolbar">
        ...
        >
          ...
          >
            {views.map((view) => (
              <MenuItem value={view}>{messages[view]}</MenuItem>
            ))}
          </Select>
          {width < 1200 && showFilters && FilterComp && (
            <IconButton
              onClick={() => setOpen(true)}
              style={{ marginLeft: 30 }}
            >
              <OutlinedIcon>filter_list</OutlinedIcon>
            </IconButton>
          )}
        </div>
    ...
    <MuiThemeProvider theme={theme}>
      ...
      >
        <StaticCalendar
          ...
          onSelectEvent={(e) => {
            /*let dt = dv.filter(d=>d[titleKey] === e.title)[0];*/ console.log(
              e
            );
        ...
        >
          ...
          />
          <div style={{width:320}}>
            {showFilters && FilterComp && FilterComp}
          </div>
        </div>
        <Drawer
          open={width < 1200 && open}
          onClose={() => setOpen(false)}
          anchor="right"
        >
          {showFilters && FilterComp && (
...
CalendarUnit.propTypes = {
  ...
  /**
   * Whether filters should be shown or not
   */
