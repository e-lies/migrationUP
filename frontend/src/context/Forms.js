...
export default function FormsContext({ children }){
	...
	useEffect(()=>{
		console.log("forms has changed: ",forms)
	...
	const addForm = (type,rule,prop) =>{
		...
		console.log("forms = ",forms,formProps)
		setForms(forms.concat(formProps))
	}
	const removeForm = (type,rule) =>{
		console.log("remove forms = ",forms)
	...
	return(
      ...
      >
      	{forms.map(frm=>{
			...
			return(<div id={`${frm.type}-${frm.rule}`}>
				<Alerts id={`${frm.type}_${frm.rule}`}
					...
					headerFooterFormat={typeHeader[form.type](form,context.schemas[typeKeys[frm.type]][frm.rule] || {})}
				>
					<Formulaire {...form} />
				</Alerts>
				</div>
