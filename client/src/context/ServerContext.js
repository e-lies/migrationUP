...
export default class ServerContext extends React.Component {
  state = {
    ...
    schemaAttempts: {},
    schemas: localStorage.getItem("schemas")
      ? JSON.parse(localStorage.getItem("schemas"))
      : { select: {}, insert: {}, update: {}, delete: {} },
    data: {},
    writeCache: localStorage.getItem("writeCache")
      ? JSON.parse(localStorage.getItem("writeCache"))
      : { insert: [], update: [], delete: [], inserts: [] },
  ...
  componentWillUnmount() {
    console.log("unmount server context");
  ...
  read = (quer, idComp = "", force = false, callback = null) => {
    //Attention! S'il y a plusieurs requetes, callback s'éxécutera sur le résultat de chacune
    let tm = new Date().getTime();
    let { data } = this.state;
    ...
    queries.map((q, i) => {
      ...
      if (
        ...
        queries.splice(i, 1);
        if (callback) {
          callback(data[q.rule][parm]["data"], q.rule, parm);
        }
      }
    });
    if (queries.length > 0) {
            ...
            prm.json().then(function(rep) {
              ...
              Object.keys(rep).map((ru) => {
                ...
                Object.keys(rep[ru]).map((param) => {
                  ...
                  if (callback) {
                    callback(rep[ru][param]["data"], ru, param);
                  }
  ...
  write = (pst, callback, idComp = "") => {
    ...
    if (common.length) {
      ...
      drop = confirm(
        "Une requète du même type a déjà été envoyée, voulez-vous la remplacer?"
      );
    }
