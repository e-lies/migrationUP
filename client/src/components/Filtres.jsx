...
export default function Filtres(props) {
  ...
  const {
    title, confirmation, inputs, keys, disabled, spacing, widthCoef, initialState, onChange, onConfirm
  } = props;
  ...
  return (
    <Grid id="filterGrid" container className={classes.grid} spacing={spacing || 0}>
      ...
      <Grid container xs={6} sm={4} md={3} lg={2} xl={1} style={{width:80,display:'flex',justifyContent:'space-between'}}>
        <Bouttons {...prp} />
        { confirmation && <Bouttons icon="check" color="primary" callback={()=>{ onChange(state); onConfirm()}} /> }
      </Grid>
    </Grid>
  );
}
