  ...
  const barUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.X !== null && obj.Y !== null && obj.op !== null) {
      ...
      if(newSchema.columns[obj.X]['type'].includes('foreign')){
        dt = dt.map(d=>{
          return {...d,
            [obj.X]:newSchema.columns[obj.X].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.X])}) ? newSchema.columns[obj.X].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.X])})['label'] : d[obj.X]
            }
          })
      }
